...
// exactly those and reports what it removed. It cannot touch a real record
// because the match is anchored at the start of the row's own identifier and
// no real identifier in this system begins with ZZTEST- : module IDs begin
// with the module prefix (KRA/KPI/GOAL/TRN/TRP/TS), task IDs are generated
// client-side from a timestamp, App Data keys always begin with be_, and a
// real cid begins with 'c'. It never runs unless the owner key is supplied.
//
// §7 requires that test data be removed "through a safe scripted cleanup
// method, never through risky manual spreadsheet clicks" — this is that
// method, and it is the ONLY thing the smoke test uses to tidy up after
// itself. Nothing here bulk-cleans production data (§2 Bug #10): a row is
// deleted only when it carries the reserved prefix that the smoke test alone
// writes.
...
function beCleanupTestRecords(ownerKey) {
  ...
  // 1. Module sheets (§§9–18) — matched on the 'ID' header wherever it sits.
  Object.keys(BE_MODULES).forEach(function (k) {
    const m = BE_MODULES[k];
  ...
  // 2. Tasks — §7 check 7 round-trips a task through the real task_sync write
  //    path, so the smoke test can leave a row in a sheet that predates the
  //    module registry and is therefore invisible to the loop above.
  //    Column 0 is 'Task ID' (see TASK_HEADERS); the header is read rather
  //    than assumed, so a future column insert cannot silently retarget this.
  const tsh = ss.getSheetByName(SH.TASKS);
  if (tsh && tsh.getLastRow() >= 2) {
    const TH = beHeaderMap(tsh);
    const tCol = (TH['Task ID'] === undefined) ? 0 : TH['Task ID'];
    const nT = deleteRowsWhere(tsh, function (row) {
      return String(row[tCol] || '').indexOf(BE_TEST_ID_PREFIX) === 0;
    });
    if (nT) removed[SH.TASKS] = nT;
  }

  // 3. App Data — §7 checks 8, 9, 10, 14 and 15 round-trip documents through
  //    the real appdata write path. Every genuine key begins with 'be_', so a
  //    ZZTEST- key can only have come from the smoke test.
  const ash = ss.getSheetByName(SH.APPDATA);
  if (ash && ash.getLastRow() >= 2) {
    const AH = beHeaderMap(ash);
    const aCol = (AH['Key'] === undefined) ? 0 : AH['Key'];
    const nA = deleteRowsWhere(ash, function (row) {
      return String(row[aCol] || '').indexOf(BE_TEST_ID_PREFIX) === 0;
    });
    if (nA) removed[SH.APPDATA] = nA;
  }

  // 4. Submit Log — the cid diagnostics written by logCid for the test posts
  //    above. Real cids are generated by genCid() as 'c'+base36, so they can
  //    never match. The audit trail for real submissions is untouched.
  const lsh = ss.getSheetByName(SH.SUBMITLOG);
  if (lsh && lsh.getLastRow() >= 2) {
    const LH = beHeaderMap(lsh);
    const lCol = (LH['CID'] === undefined) ? 1 : LH['CID'];
    const nL = deleteRowsWhere(lsh, function (row) {
      return String(row[lCol] || '').indexOf(BE_TEST_ID_PREFIX) === 0;
    });
    if (nL) removed[SH.SUBMITLOG] = nL;
  }
